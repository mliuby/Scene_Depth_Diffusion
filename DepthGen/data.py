...
def loadZipToMem(zip_file):
    ...
    nyu2_train = [row.rstrip('\r\n').split(',') for row in (data['data/nyu2_train.csv']).decode("utf-8").split('\n') if len(row) > 0]
...
class To_Tensor(object):
    ...
    def __call__(self, sample):
        ...
        depth = depth.resize((640, 480)) #w=640,h=480
...
'''
trainloader, testloader = get_data_loaders("data/nyu_depth.zip", batch_size=4)
for batch in testloader:
    print(batch["image"].shape, batch["depth"].shape)
    break
'''
#torch.Size([4, 3, 480, 640]) torch.Size([4, 1, 480, 640])
